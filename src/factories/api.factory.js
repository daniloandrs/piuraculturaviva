...
dnInjectable({
    ...
    fn: function (sApi, q) {
        return {
            ...
            getPublic : route => {
                return sApi.getPublic(route);
            },
            postPublic : (route,data) => {
                return sApi.postPublic(route,data);
            },
            post: (route, data) => {
            ...
            download: function (route, data, typeFile, nombre,isPublic) {
                return sApi.download(route, data, typeFile, nombre,isPublic);
            },
        };
    },
    deps: ['sApi', '$q']
...
dnInjectable({
    ...
    fn: function (sApiToken) {
        ...
        this.getPublic = (route) => {
            return sApiToken.getPublic(route);
        };
        this.postPublic = (route,data) => {
            return sApiToken.postPublic(route,data);
        };
        this.post = (route, data) => {
            return sApiToken.post(route, data);
        };
        this.download = (route, data, typeFile, nombre,isPublic) => {
            return sApiToken.download(route, data, typeFile, nombre,isPublic);
        };
    },
    deps: ['sApiToken']
});
...
dnInjectable({
    name: 'sApiToken',
    module: 'mdFactories',
    fn: function (http, cApi, sMiddleware, queue) {
        ...
        this.download = function (route, data, typeFile, nombre, isPublic) {
            let promise = new Promise((resolver, reject) => {
                console.log(isPublic,route)
                    
                let response = http({
                    
                    url: isPublic ? route : server + route,
                    method: 'POST',
                    data: data,
                    body: data, 
                    headers: {
