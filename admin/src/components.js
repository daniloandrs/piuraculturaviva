...
const dnNavbar = dnComponent({
    name: 'navbar',
    fn: function (scope, sRoot, sModal, storage, cSession, fUser, modal, ffirebase, pTranslate) {

        const languajePanel = document.querySelector('.languaje-panel')

        scope.isClient = storage.get(cSession.MENU).nombre === 'Cliente'
        pTranslate.use(scope.isClient ? 'en' : 'sp')

        scope.toggleSidebar = () => {
            let elem = angular.element('#toggle-sidebar');
            if (elem) {
                scope.sidebar = elem[0];
                elem.prop('checked', !elem[0].checked);
                sRoot.set('state-sidebar', elem[0].checked);
            }
        };

        scope.openModal = () => {
            sModal.question('¿Desea salir?', () => {
                sModal.close();
                let token = storage.get(cSession.TOKEN);
                storage.clear();
        ...
        scope.onInit = () => {
            scope.user = storage.get(cSession.USER)
        };

        /** Mail Box */
        scope.mailbox = {
            // news: ffirebase.mailboxNews,
            notificaciones_leidas: (ffirebase.notificaciones_leidas),
            notificaciones_no_leidas: (ffirebase.notificaciones_no_leidas),
        };
        scope.show_mailbox = false;

        scope.toggleMailbox = () => {
            scope.show_mailbox = !scope.show_mailbox;
        };

        scope.toggleDontRead = () => {
        ...
        let checkedRead = item => {
            ...
            scope.mailbox.notificaciones_no_leidas.$remove(item).then(() => {
                let new_item = angular.copy(item)
                new_item.leido = true
                scope.mailbox.notificaciones_leidas.$add(new_item);
            })
        }

        scope.openMessage = mail => {
            scope.current_message = angular.copy(mail);

            modal.open('modal-show-message-navbar', () => {
                checkedRead(mail);
            });
        };

        scope.openAllMessages = () => {
            modal.open('modal-show-messages-all');
            scope.show_mailbox = false;
            scope.current_message = undefined;
        };

        scope.selectMessage = mail => {
            scope.current_message = mail;
        };

        scope.deleteMessage = mail => {
            modal.question('Se borrará permanentemente este mensaje, ¿continuar?', () => {
                let message = mail ? mail : scope.current_message;
                scope.mailbox.olds.$remove(message);
                scope.current_message = undefined;
                modal.close();
            });
        };

        scope.openLanguajePanel = value => {
            if (value !== undefined) {
                if (value) languajePanel.classList.add('show')
                else languajePanel.classList.remove('show')
                return
            }
            languajePanel.classList.toggle('show')
        }
    },
    deps: [
        '$scope',
    ...
    // templateUrl: './src/components/system/navbar/navbar.html',
    // stylesUrl: './src/components/system/navbar/navbar.css',
    template: `<nav class="navbar navbar-expand-lg fixed-top" ng:init="onInit()">
    <div class="container-fluid">
        <ul class="navbar-nav">
            <li class="nav-item" ng:click="toggleSidebar()">
                <a class="nav-link" href="" app-tooltip="{{sidebar__state.value?'Ocultar':'Mostrar'}} el sidebar">
                    <i class="fa" ng:class="sidebar__state.value?'fa-ellipsis-v':'fa-th-list'">
                    </i>
                </a>
            </li>
        ...
        <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle" href="" id="malbox-dropdown" data-toggle="dropdown" aria-haspopup="true"
                    aria-expanded="false" app-tooltip="Tienes {{mailbox.notificaciones_no_leidas.length}} mensajes no leídos">
                    <i class="fa fa-envelope"></i>
                    <span ng:bind="mailbox.notificaciones_no_leidas.length"></span>
                </a>
                <div class="dropdown-menu" aria-labelledby="malbox-dropdown" style="right: 0; left: initial">
                    <ul class="mailbox list-unstyled" ng:class="{'scroll-y':mailbox.notificaciones_no_leidas.length > 0}">
                        <li class="mailbox-header" ng:if="mailbox.notificaciones_no_leidas.length > 0">
                            <small>
                                MENSAJES NO LEÍDOS
                            </small>
                        </li>
                        <li class="text-center mailbox-empty" ng:if="mailbox.notificaciones_no_leidas.length === 0">
                        ...
                        <li class="mailbox-item" ng:click="openMessage(mail)" ng:repeat="mail in mailbox.notificaciones_no_leidas"
                            style="cursor: pointer">
                            <img ng:if="mail.emisor.image" class="img" alt="" ng:src="{{mail.emisor.image}}">
                            <div class="mailbox-pre-content">
                                <small class="text-muted">
                                    <i class="fa fa-user">
                                    </i>
                                    <span ng:bind="mail.emisor">
                                    </span>
                                </small>
                                <small style="color: #17a2b8">
                                    <i class="fas fa-closed-captioning">
                                    </i>
                                    <span ng:bind="mail.asunto">
                                    </span>
                                </small>
                                <small>
                                    <i class="fa fa-envelope">
                                    </i>
                                    <span ng:bind-html="mail.mensaje | fCutMessage">
                                    </span>
                                </small>
                                <small>
                                    <i class="far fa-clock">
                                    </i>
                                    <span am-time-ago="mail.fecha | amFromUnix">
                                    </span>
                                </small>
                            </div>
                        </li>
                        ...
                        <li class="text-center see-all">
                            <button class="btn btn-info btn-sm" ng:click="openAllMessages(1)">
                                VER TODO
                            </button>
                        </li>
                    </ul>
            ...
            -->
            <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle" href="" id="user-config-dropdown" data-toggle="dropdown"
                    aria-haspopup="true" aria-expanded="false" app:tooltip="Opciones de usuario">
                    <i class="fa fa-user"></i>
                </a>
                <div class="dropdown-menu" aria-labelledby="user-config-dropdown" style="right: 0; left: initial">
                    <a class="dropdown-item" href="#/perfil">
                        <i class="fa fa-user"></i> Perfil</a>
                    <a class="dropdown-item" href="#/cuenta">
                        <i class="fa fa-address-card"></i> Actualizar datos</a>
                    <a class="dropdown-item" href="" ng:click="openModal()">
...
<dn-modal name="modal-show-message-navbar" size="lg" title-modal="{{isClient?'View':'Detalles del mensaje'}} - {{current_message.emisor}}">
...
<dn-modal class="modal-all-messages" name="modal-show-messages-all" size="lg" title-modal="Todos mis mensajes ({{mailbox.notificaciones_leidas.length}})"
    scale="12">
    <div class="container-fluid">
        <div class="row">
            <div class="col-5 scroll-y" style="min-height: calc(100vh - 115px); padding: 0;">
                <ul class="list-unstyled">
                    <li class="list-item mailbox-item" style="min-height: initial !important;">
                        <div class="form-group" style="margin-bottom: 0;">
                            <input class="form-control bg-white" ng:model="filter_message" placeholder="Escribe para buscar"
                                style="font-size: 12px !important;" type="text">
                            </input>
                        </div>
                    </li>
                    <li class="list-item mailbox-item" ng:class="{'active':mail === current_message}" ng:focus="selectMessage(mail)"
                        ng:repeat="mail in mailbox.notificaciones_leidas | orderBy: 'fecha' : 'DESC' | filter:filter_message"
                        style="min-height: initial !important;" tabindex="1">
                        <img ng:if="mail.emisor.image" alt="" class="img img-raised" ng:src="{{mail.emisor.image}}"
                            width="40">
                        <div class="mailbox-pre-content" style="width: 74%;">
                            <small ng:bind="mail.asunto | fCutMessage" style="margin-bottom: 0;">
                            </small>
                            <small am-time-ago="mail.fecha | amFromUnix">
                            </small>
                        </div>
                    </li>
                </ul>
            </div>
            <div class="col-7 scroll-y" style="min-height: calc(100vh - 115px); padding: 0;">
                <div class="modal-mailbox">
                    <div class="modal-mailbox-content">
                        <div class="general-preview-content">
                            <div ng:if="current_message" d-html-compile="current_message.mensaje"></div>
                        </div>
                    </div>
...
@media screen and (max-width: 991px) {
    ...
    .navbar .navbar-brand .nav-description {
        margin-right: 25px;
    }
...
.mailbox-item .mailbox-pre-content {
    display: inline-block;
    margin-left: 10px;
    width: 75%;
}

.mailbox-item small:first-child {
    font-weight: bold;
    margin-bottom: 5px;
}

.mailbox-item small:nth-child(2) {
    /*border-bottom: 1px solid #E4ECF3;*/
    margin-bottom: 5px;
}

.mailbox-item small:nth-child(4) {
    margin-top: 10px;
    font-weight: bold;
}
...
.navbar .mailbox li.mailbox-empty i {
    display: block;
    margin: 30px;
    font-size: 50px;
}

.navbar .mailbox li.see-all {
    display: flex;
...
.navbar .mailbox li.mailbox-empty small,
.navbar .mailbox li.mailbox-header small {
    display: block;
    font-size: 10px;
...
.navbar .mailbox li.mailbox-header {
    padding-left: 5px;
}
...
.navbar .mailbox li:last-child button {
    font-size: 10px;
}
...
.modal-mailbox .modal-mailbox-icon {
    justify-content: center;
...
.modal-mailbox .modal-mailbox-icon img {
    width: 80px;
}

.modal-mailbox .modal-mailbox-content {}

.modal-mailbox .modal-mailbox-content span.modal-mailbox-title,
.modal-mailbox .modal-mailbox-content span.modal-mailbox-source,
.modal-mailbox .modal-mailbox-content span.modal-mailbox-body {
    display: block;
}
...
.modal-all-messages .modal-body {
    padding: 0 !important;
}

.modal-all-messages .btn-action,
.modal-mailbox .btn-action {
    position: absolute;
    right: 13px;
    top: 4px;
}
...
.container-languaje .languaje-title {
    padding: .25rem 0 1.5rem;
    font-size: 1.3em;
    color: #353535;
}

.container-languaje .languaje-subtitle {
    color: #33691E;
    font-size: 1.15em;
}

.container-languaje select {
    width: 100%;
...
.languaje-panel.show .container-languaje {
    right: 0;
}`
});
...
const dnImageOnLoad = dnDirective({
    ...
    fn: function() {
        ...
        console.log(scope.dnAttrs);
        
        scope.dnElement.bind('load', function() {
            scope.$apply(scope.dnAttrs)(true);
        });
        scope.dnElement.bind('error', function(){
            scope.$apply(scope.dnAttrs)(false);
        });
    }
